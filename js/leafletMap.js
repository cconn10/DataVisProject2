class LeafletMap {
  ...
  initVis() {
    ...
    vis.tileLayer2 = L.tileLayer(vis.esriUrl, {
      id: "esri-image",
      attribution: vis.esriAttr,
      ext: "png",
    });

    vis.tileLayer3 = L.tileLayer(vis.topoUrl, {
      id: "topo-image",
  ...
  updateVis() {
    ...
    vis.Squares = vis.svgKey
      ...
      .attr("y", function () {
        return (vis.keyY = vis.keyY + 15);
      })
      .attr("width", 10)
      .attr("height", 10)
      .attr("fill", function (d) {
        return vis.keyScale(d);
      })
      .attr("stroke", "black");
    ...
    vis.data.dayTally = new Array(
      d3.timeDay.count(vis.data.timeBounds[0], vis.data.timeBounds[1]) + 1
    ).fill(0);

    // Max # of calls shown for each day
    vis.data.dayMax =
      250 / d3.timeDay.count(vis.data.timeBounds[0], vis.data.timeBounds[1]);

    vis.filteredData = vis.data.filtered.filter(
      (d) =>
        d.requestedDate >= vis.data.timeBounds[0] &&
        d.requestedDate <= vis.data.timeBounds[1]
    );
    ...
    vis.filteredData = vis.filteredData.filter((d) => {
      let index = d3.timeDay.count(
        vis.data.filtered[0],
        vis.data.parseTime(d.REQUESTED_DATETIME)
      );
    ...
    vis.Dots = vis.svg
      .selectAll("circle")
      .data(
        vis.filteredData.filter((d) => {
          return (
            !isNaN(d.longitude) &&
            !isNaN(d.latitude) &&
            vis.data.timeBounds[0] <=
              vis.data.parseTime(d.REQUESTED_DATETIME) &&
      ...
      .join("circle")
      .attr("fill", function (d) {
        return vis.colorScale(vis.getColorInput(d));
      })
      .attr("stroke", "black")
  ...
  getKeyDescription() {
    ...
    if (this.color == "service") {
      ...
      const serviceArr = Array.from(serviceData, function (d) {
        return { key: d[0], value: d[1] };
      });
      //console.log(planetArr);

      let sorteService = serviceArr
        .slice()
      ...
      let agencies = d3.rollup(
        vis.data,
        (v) => v.length,
        (d) => d.AGENCY_RESPONSIBLE
      );
      //console.log(planetData);

      const agenciesArr = Array.from(agencies, function (d) {
        return { key: d[0], value: d[1] };
      });
      //console.log(planetArr);

      let sortedAgencies = agenciesArr
        .slice()
  ...
  setColorScale() {
    ...
    if (vis.color == "service") {
      ...
      return d3
        .scaleOrdinal()
        .domain(sorteService.map((d) => d.key))
        .range([
          "#4e79a7",
          "#f28e2c",
          "#e15759",
      ...
      return d3
        .scaleOrdinal()
        .domain(sortedAgencies.map((d) => d.key))
        .range([
          "#4e79a7",
          "#f28e2c",
          "#e15759",
